...
from utilities.djangologging import SUPPRESS_OUTPUT_ATTR, getLevelNames
from utilities.djangologging.handlers import ThreadBufferedHandler
...
LOG_FILENAME = settings.DIRNAME + '/logs/console_logger.out'

handler = ThreadBufferedHandler()
logging.root.setLevel(logging.NOTSET)
logging.root.addHandler(handler)

# Check if the LOGGING_CONSOLE is enabled. If Yes then enable the 
# RotatingFileHandler.
try:
    console_logging = settings.LOGGING_CONSOLE
    if console_logging:
        import logging.handlers
        rotating_handler = logging.handlers.RotatingFileHandler(
                            LOG_FILENAME, maxBytes=2000000, backupCount=20)
